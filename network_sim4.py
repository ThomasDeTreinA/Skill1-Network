...
class MissionSystem:
    ...
    def check_conditions(self, devices, connections, packets, sm=None, active_window=None, active_device=None, isp_inputs=None):
        ...
        if mission.type == "PLACE":
            ...
            # R1 <-> R2 (Crossover)
            r_r = any(c.is_valid and c.cable_type == 'Crossover' and c.d1.type == 'Router' and c.d2.type == 'Router' for c in connections)
            if r_r: self.advance()

        elif mission.type == "L3_H2_LAP":
            # Laptop with Wi-Fi connection
            if any(c.is_valid and c.cable_type == 'Wi-Fi' and (c.d1.type == 'Laptop' or c.d2.type == 'Laptop') for c in connections):
                self.advance()

        elif mission.type == "L3_H2_IPS":
            # PC(192.168.2.2), R1(192.168.2.1), R2(192.168.3.1)
            pc = next((d for d in devices if d.type == 'PC'), None)
            r1 = next((d for d in devices if d.type == 'Router'), None)
            r2 = next((d for d in devices if d.type == 'Router' and d != r1), None)
            if pc and r1 and r2:
                ips = {pc.ip, r1.ip, r2.ip}
                if "192.168.2.2" in ips and "192.168.2.1" in ips and "192.168.3.1" in ips:
                    self.advance()
                    self.packets_delivered = 0 # Reset for final test
